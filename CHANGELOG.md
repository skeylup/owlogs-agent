...
- `owlogs:install` artisan command — guided installer: validates the workspace API key against the ingest endpoint, writes/updates the `OWLOGS_*` block in `.env` diff-aware, auto-detects the buffer store (redis when reachable, file otherwise), optionally emits test logs and finishes by running `owlogs:doctor`.
- `owlogs:doctor` artisan command — diagnoses every link of the shipping chain in one pass (API key, ingest endpoint, buffer store, cache debounce, queue connection, worker probe, ingest circuit, pending backlog). Exits non-zero only on failed checks, safe to wire into CI. `--reset-circuit` closes a tripped ingest circuit.
- `owlogs.redaction` config — central PII scrubbing for everything the agent captures (request input, log context/extra, Livewire call params, model-change attributes): `key_patterns` (case-insensitive key substrings, whole value masked) and `value_regexes` (PCRE applied to every captured string value), replacing the lists previously hardcoded in `AddLogContext`, `AutoLogger` and `OwlogsLivewireHook`.
- `owlogs.sampling` config — volume dials enforced before buffering: per-level keep probabilities (`OWLOGS_SAMPLE_*`) and per-URI-pattern trace sampling derived deterministically from the `trace_id` (a kept trace keeps all its rows, a dropped trace disappears entirely).
